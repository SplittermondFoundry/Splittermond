...
export async function importNpc(rawData: string) {
    ...
    let tokenizedData = rawString.split(
        /(AUS\s+BEW\s+INT\s+KON\s+MYS\s+STÄ\s+VER\s+WIL\s+GK\s+GSW\s+LP\s+FO\s+VTD\s+SR\s+KW\s+GW|GK\s+GSW\s+LP\s+FO\s+VTD\s+SR\s+KW\s+GW|AUS\s+BEW\s+INT\s+KON\s+MYS\s+STÄ\s+VER\s+WIL|AUS\s+BEW\s+INT\s+KON\s+MYS\s+STÄ\s+VER\s+WIL\s+GK\s+GSW\s+LP\s+FO\s+VTD\s+SR\s+KW\s+GW|Fertigkeiten:|Monstergrad:|Zauber:|Meisterschaften:|Merkmale:|Beute:|Wichtige Attribute:|Wichtige abgeleitete Werte:|Waffen Wert Schaden WGS.*|Kampfweise:|Typus:|Übliche Anzahl:|Anmerkung:|Besonderheiten:)/
    );
        ...
        try {
            switch (true) {
                ...
                case /GK\s+GSW\s+LP\s+FO\s+VTD\s+SR\s+KW\s+GW/.test(tokenizedData[i]):
                    let derivedValueData = tokenizedData[i + 1]
                        .trim()
                        .match(/([0-9]+(:?\s+\/\s+[0-9]+)*)/g)
                ...
                case /Fertigkeiten:/.test(tokenizedData[i]):
                    tokenizedData[i + 1]
                        ...
                        .forEach((skillStr) => {
                            ...
                            let skill = splittermond.skillGroups.all.find(
                                (i) =>
                                    foundryApi.localize(`splittermond.skillLabel.${i}`).toLowerCase() ===
                                    skillData[1].trim().toLowerCase()
                            );
                            ...
                            if (skill) {
                                ...
                                if (isNoFightingSkill(skill) && splittermond.skillAttributes[skill]) {
                                    ...
                                    actorData.system.skills[skill].points -=
                                        parseInt(
                                            actorData.system.attributes[splittermond.skillAttributes[skill][0]].value
                                        ) || 0;
                                    actorData.system.skills[skill].points -=
                                        parseInt(
                                            actorData.system.attributes[splittermond.skillAttributes[skill][1]].value
                                        ) || 0;
                                    if (skill === "stealth") {
                                        actorData.system.skills[skill].points -=
                                            5 - parseInt(actorData.system.derivedAttributes.size.value);
                                    }
                                }
                            }
                        });
                ...
                case /Waffen Wert Schaden WGS.*/.test(tokenizedData[i]):
                    ...
                    let weaponData = (tokenizedData[i + 1].trim().match(new RegExp(weaponRegExpStr, "g")) ?? []).map(
                        (weaponStr) => {
                            weaponStr = weaponStr
                                .trim()
                                .replace(/\s/g, " ")
                                .replace(/\s{2,}/g, " ");
                            let weaponMatch = weaponStr.match(
                                new RegExp(
                                    `(.*?)\\s+([0-9]+)\\s+([0-9W+\\-]+)\\s+([0-9]+)(?:\\s+Tick[s]?)?\\s+([\\-–]+|[0-9]+|[0-9]+\\s*m)?\\s?([0-9]+)\\-1?W6\\s*(.*)`
                                )
                            );
                    ...
                    actorData.items.push(
                        ...(await Promise.all(
                            weaponData.map(async (data) => {
                                let weaponData = data.name
                                    ? await SplittermondCompendium.findItem("weapon", data.name)
                                    : null;
                                if (!weaponData) {
                                    weaponData = {
                                        type: "npcattack",
                                ...
                                if (weaponData.type == "npcattack") {
                                    ...
                                    actorData.system.skills[weaponData.system.skill] = {
                                        ...
                                        points:
                                            data.skillValue -
                                            actorData.system.attributes[weaponData.system.attribute1].value -
                                            actorData.system.attributes[weaponData.system.attribute2].value,
                                    };
                                }
                                return weaponData;
                            })
                ...
                case /Meisterschaften:/.test(tokenizedData[i]):
                    ...
                    tokenizedData[i + 1]
                        ...
                        ?.forEach((skillEntryStr) => {
                            ...
                            let skill = [
                                ...
                                (i) =>
                                    foundryApi.localize(`splittermond.skillLabel.${i}`).toLowerCase() ===
                                    masteryEntryData[1].trim().toLowerCase()
                            );
                            let level = 1;
                    ...
                    actorData.items.push(
                        ...(await Promise.all(
                            masteries.map(async (data) => {
                                let searchString = data.name.match(/([^(,\[]+)(?:\([^)]+?\)|\[[^\]]+?\])?/);
                                let masteryData = await (searchString
                                    ? SplittermondCompendium.findItem("mastery", searchString[1].trim())
                                    : null);
                                if (masteryData) {
                    ...
                    actorData.items.push(
                        ...(await Promise.all(
                            features.map(async (data) => {
                                ...
                                let featureData = await (searchString
                                    ? SplittermondCompendium.findItem("npcfeature", searchString[1].trim())
                                    : null);
                                if (featureData) {
                ...
                case /Zauber:/.test(tokenizedData[i]):
                    ...
                    tokenizedData[i + 1]
                        ...
                        ?.forEach((skillEntryStr) => {
                            ...
                            if (spellEntryData[2]) {
                                let newSkill = splittermond.skillGroups.magic.find((i) =>
                                    foundryApi
                                        .localize(`splittermond.skillLabel.${i}`)
                                        .toLowerCase()
                                        .startsWith(spellEntryData[2].toLowerCase())
                                );
                                if (newSkill) {
                                    skill = newSkill;
                                }
                    ...
                    actorData.items.push(
                        ...(await Promise.all(
                            spells.map(async (data) => {
                                let searchString = data.name.match(/([^\[]+)(?:\[[\[]+\])?/);
                                let spellData = await (searchString
                                    ? SplittermondCompendium.findItem("spell", searchString[1])
                                    : null);
                                if (spellData) {
            ...
            console.log(
                foundryApi.format("splittermond.error.parseError", { section: tokenizedData[i] }) + tokenizedData[i + 1]
            );
        }
    }
