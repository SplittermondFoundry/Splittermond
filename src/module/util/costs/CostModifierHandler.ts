...
type ValidMapper = Parameters<
  ReturnType<typeof normalizeDescriptor>["usingMappers"]
>[0];
...
export class CostModifierHandler extends ModifierHandler<FocusModifier> {
    ...
    return [
      {
        ...
        skill:
          "skill" in this.sourceItem.system
            ? this.sourceItem.system.skill
            : null,
        attributes: {
  ...
  private normalizeAttribute(
    ...
    return validated
      ? normalizeDescriptor(validated).usingMappers(mapper).do()
      : validated;
  }
  ...
  private normalizeSkill(groupId: string, input: Value) {
    ...
    if (normalized && !isMember(splittermond.skillGroups.magic, normalized)) {
      ...
      return hasSkill(this.sourceItem.system)
        ? this.sourceItem.system.skill
        : normalized;
    }
    return normalized;
  ...
  private getMultiplier(groupId: string): Expression {
    ...
    if (
      lowerCaseId.endsWith("reduction") ||
      lowerCaseId.endsWith("enhancedreduction")
    ) {
