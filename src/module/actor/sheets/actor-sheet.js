...
export default class SplittermondActorSheet extends SplittermondBaseActorSheet {
    ...
    constructor(options) {
        const instanceDefaults = {
            actions: {
                ...
                "end-channel": (_e, t) => this.#handleEndChannel(t),
                "inc-damage": (_e, t) => this.#handleIncDamage(t),
    ...
    async _prepareContext(options) {
        ...
        // Prepare Active Effects for the status tab display
        sheetData.activeEffects = Array.from(this.actor.allApplicableEffects())
            .map((e) => ({
                id: e.id,
                uuid: e.uuid,
                name: e.name,
                ...
                autoGenerated: e.type === "autoGenerated",
                sort: e.sort ?? 0,
                ...buildEffectCardContext({
                    isSuppressed: e.isSuppressed,
                    disabled: e.disabled,
                    type: e.type,
                    durationMode: e.durationMode,
                    duration: e.duration,
                    isIneffective: this.actor.isEffectIneffective(e),
                }),
            }))
    ...
    /**
     * Handle deleting an array element; channeled entries are reverted without conversion
     * @param {HTMLElement} target - The target element
     */
    #handleDeleteArrayElement(target) {
        ...
        if (address === "system.health.channeled.entries") {
            return this.actor.removeChannel("health", idx);
        }
        if (address === "system.focus.channeled.entries") {
            return this.actor.removeChannel("focus", idx);
        }

        value.splice(idx, 1);
        return this.actor.update({ [address]: value });
    }

    /**
     * Handle ending a channeled entry, converting its costs into exhaustion
     * @param {HTMLElement} target - The target element
     */
    #handleEndChannel(target) {
        const idx = parseInt(closestData(target, "index", "0"));
        const { address } = this.#getArray(target);

        if (address === "system.health.channeled.entries") {
            return this.actor.endChannel("health", idx);
        }
        if (address === "system.focus.channeled.entries") {
            return this.actor.endChannel("focus", idx);
        }
    }

    /**
     * Handle incrementing a damage counter, absorbing from the bonus pool first
     * @param {HTMLElement} target - The target element
     */
    #handleIncDamage(target) {
        const type = closestData(target, "type");
        const counter = closestData(target, "counter");
        if (type !== "health" && type !== "focus") return;
        if (counter !== "consumed" && counter !== "exhausted") return;
        const cost = counter === "consumed" ? new Cost(0, 1, false) : new Cost(1, 0, false);
        return this.actor.applyCost(type, cost.asPrimaryCost(), "");
    }
    ...
    #handleConsume(target) {
        ...
        if (type === "focus") {
            ...
            this.actor.applyCost(type, parseCostString(value ?? "").asPrimaryCost(), description ?? "");
        }
    }
     ...
     */
    #handleAddEffect() {
        SplittermondActiveEffectCreationDialog.show(SplittermondActiveEffect, {}, { parent: this.actor });
    }

    #handleEditEffect(target) {
        const uuid = closestData(target, "effect-uuid");
        if (!uuid) return;
        const effect = foundryApi.utils.fromUUIDSync(uuid);
     ...
     */
    async #handleToggleEffect(target) {
        const uuid = closestData(target, "effect-uuid");
        if (!uuid) return;
        const effect = foundryApi.utils.fromUUIDSync(uuid);
        if (!effect) return;
     ...
     * @param {HTMLElement} target
     */
    async #handleDeleteEffect(target) {
        const uuid = closestData(target, "effect-uuid");
        if (!uuid) return;
        const effect = foundryApi.utils.fromUUIDSync(uuid);
        if (!effect) return;
    ...
    _onDragStart(event) {
        ...
        const effectUuid = target.dataset.effectUuid ?? target.dataset.uuid;
        if (effectUuid) {
            const effect = foundryApi.utils.fromUUIDSync(effectUuid);
            if (!effect) return;
    ...
    /**
     * @param {DragEvent} _e
     * @param {SplittermondItem} document
    ...
    async _onDropItem(event, document) {
        ...
        const newDocument = await super._onDropItem(event, document);
        if (!newDocument) return newDocument;
