...
type SubSegments<T> =
    T extends Record<string, any> ? Readonly<{ [K in keyof T]: Readonly<T[K] & ConfigSegment> }> : never;
...
export function makeConfig<CONFIG extends PartialConfig>(
    proto: CONFIG
): CONFIG extends { subSegments: infer S }
...
function makeSubSegments<T extends Record<string, any>>(
    proto: T
): { readonly [K in keyof T]: Readonly<T[K] & ConfigSegment> } {
    const mappedEntries = Object.entries(proto).map(([key, value]) => [key, makeConfigSegment(value)]);
