...
interface EffectsPartContext {
    effectsType: "modifier" | "base";
    rawInput?: string;
    modifierHelpText?: string;
...
export class SplittermondActiveEffectConfig extends FoundryActiveEffectConfig {
    ...
    async #prepareEffectsContext(
        context: ApplicationRenderContext
    ): Promise<ApplicationRenderContext & EffectsPartContext> {
        const effect = this.document;
        const effectType = effect.type;
        const isAutoGenerated = isGenerated(effect);
        if (this.#isModifierType(effectType)) {
            const rawInput = effect.getFlag("splittermond", "rawInput");
            const enriched = await foundryApi.utils.enrichHtml(
            ...
            return {
                ...context,
                effectsType: "modifier",
                rawInput: (typeof rawInput === "string" && rawInput.trim()) || "",
                modifierHelpText: enriched,
                isAutoGenerated,
            };
        }

        return { ...context, effectsType: "base" };
    }
