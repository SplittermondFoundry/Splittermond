...
export function showInDefaultView(effect: DefaultViewEffect): boolean {
    if (!effect.fromItem) return true;
    if (effect.durationMode !== "permanent") return true;
    return effect.sourceItemType === "statuseffect" && !effect.autoGenerated;
}
...
export function buildEffectCardContext(effect: EffectCardEffect): EffectCardContext {
    ...
    const isAutoGenerated = effect.type === "autoGenerated";
