...
function assembleEffectData(
    system: object,
    origin: string,
    rawFragment: string,
    type: EffectType,
    extraSplittermondFlags: Record<string, unknown> = {}
): EffectCreationData {
    return {
        name: rawFragment,
        origin,
...
export function buildScalarEffectData(
    ...
    return assembleEffectData(system, origin, rawFragment, type, extraSplittermondFlags);
}
...
function effectDataFromAddResult(
    ...
    return [
        ...grouped.map(({ tagged, rawFragment }) =>
            buildScalarEffectData(tagged, item.uuid, rawFragment, type, extraSplittermondFlags)
        ),
        ...costModifiers.map(({ modifier, rawFragment }) =>
            buildCostEffectData(modifier, rawFragment, item.uuid, type, extraSplittermondFlags)
        ),
    ];
}
...
export async function addModifierEffects(
    ...
    const effectDataArray = effectDataFromAddResult(
        addModifier,
        item,
        modifierString,
        modifierType,
