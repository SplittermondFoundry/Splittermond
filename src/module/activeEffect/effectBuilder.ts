...
export function buildScalarEffectData(modifier: IModifier, rawFragment: string, origin: string): EffectCreationData {
    const system = ModifierDataModel.init(modifier.groupId, modifier.value, modifier.attributes, modifier.selectable);
    return {
        name: rawFragment,
        origin,
...
export async function addModifierEffects(
    ...
    const effectDataArray: EffectCreationData[] = [
        ...modifiers.map(({ modifier, rawFragment }) => buildScalarEffectData(modifier, rawFragment, item.uuid)),
