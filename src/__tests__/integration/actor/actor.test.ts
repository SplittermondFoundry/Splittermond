...
export function actorTest(context: QuenchBatchContext) {
    ...
    describe("Actor functions", () => {
        it(
            ...
            withActor(async (actor) => {
                ...
                expect((actor.system as CharacterDataModel).experience.heroLevel).to.equal(2);
                expect(actor.derivedValues.size.value.display, "Size value").to.equal("5");
                expect(actor.derivedValues.defense.value.display, "Defense value").to.equal("19");
                expect(actor.derivedValues.bodyresist.value.display, "Bodyresist value").to.equal("23");
                expect(actor.derivedValues.mindresist.value.display, "Mindresist value").to.equal("19");
    ...
    describe("Actor sheet", () => {
        ...
        it(
            "should switch the status tab effects between default view and all view",
            withActor(async (actor) => {
                const [statusItem] = await actor.createEmbeddedDocuments("Item", [
                    { type: "statuseffect", name: "Test Status Effect", system: { modifier: "", level: 1 } },
                ]);
                await statusItem.createEmbeddedDocuments("ActiveEffect", [
                    {
                    ...
                    { name: "Status Handcrafted", type: "modifier", transfer: true },
                ]);
                const [weapon] = await actor.createEmbeddedDocuments("Item", [
                    { type: "weapon", name: "Test Weapon", system: { equipped: true } },
                ]);
                await weapon.createEmbeddedDocuments("ActiveEffect", [
                    {
                        name: "Weapon Generated",
                        type: "autoGenerated",
                        system: { modifiers: [], costModifiers: [] },
                        transfer: true,
                    },
                ...
                await actor.createEmbeddedDocuments("ActiveEffect", [
                    { name: "Direct Disabled", type: "modifier", disabled: true },
                ]);
                await actor.createEmbeddedDocuments("ActiveEffect", [
                    { name: "Timed", type: "modifier", flags: { splittermond: { durationMode: "timed" } } },
                ]);

                actor.prepareData();

                const effectNames = (activeEffects: unknown): string[] => {
                    if (!Array.isArray(activeEffects)) {
                        expect.fail("Context should have activeEffects");
                        throw new Error("Should have activeEffects"); //Satisfy compiler
                    }
                    return activeEffects.map((e) => String((e as { name: unknown }).name));
                };

                const sheet = await new SplittermondCharacterSheet({ document: actor }).render({ force: true });
                try {
                    const defaultContext = await sheet._prepareContext();
                    const defaultNames = effectNames(defaultContext.activeEffects);
                    expect(defaultNames, "Default view shows a manual effect from a statuseffect item").to.include(
                        "Status Handcrafted"
                    );
                    expect(defaultNames, "Default view shows disabled actor-owned effects").to.include(
                        "Direct Disabled"
                    );
                    expect(defaultNames, "Default view shows timed actor-owned effects").to.include("Timed");
                    expect(defaultNames, "Default view hides auto-generated statuseffect effects").to.not.include(
                        "Status Generated"
                    );
                    expect(
                        defaultNames,
                        "Default view hides auto-generated effects from equipped items"
                    ).to.not.include("Weapon Generated");
                    expect(defaultContext.showAllEffects, "Default view flag").to.be.false;

                    const statusSection = sheet.element.querySelector('section[data-tab="status"]');
                    expect(statusSection, "Layer 1: status tab part container is rendered").to.exist;
                    expect(
                        statusSection!.querySelector(".active-effects .list-header"),
                        "Layer 2: active effects list header is rendered"
                    ).to.exist;
                    expect(
                        statusSection!.querySelector('button[data-action="show-hide-inactive-effects"]'),
                        "Layer 3: no pre-change eye button in the DOM (it would mean a stale compiled status-tab template — restart the Foundry world)"
                    ).to.be.null;
                    const toggleButton = statusSection!.querySelector<HTMLButtonElement>(
                        'button[data-action="toggle-show-all-effects"]'
                    );
                    expect(toggleButton, "Layer 4: the show-all eye button is rendered").to.exist;

                    const toggleAction = sheet.options.actions["toggle-show-all-effects"];
                    if (typeof toggleAction !== "function") {
                        expect.fail("toggle-show-all-effects action should be registered");
                    ...
                    const allContext = await sheet._prepareContext();
                    const allNames = effectNames(allContext.activeEffects);
                    expect(allNames, "All view shows every applicable effect").to.include.members([
                        "Status Generated",
                        "Status Handcrafted",
                        "Weapon Generated",
                        "Direct Disabled",
                    ...
                    expect(allContext.showAllEffects, "All view flag").to.be.true;
                } finally {
                    await sheet.close();
                }
            })
