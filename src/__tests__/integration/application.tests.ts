...
export function applicationTests(context: QuenchBatchContext) {
    ...
    describe("foundry API compatibility", () => {
        it("game.packs can be sorted by documentName", () => {
            ...
            expect(game.packs.filter((pack: any) => pack.documentName === "Item")).and.to.have.length.lessThan(
                game.packs.size
            );
        });
    ...
    describe("Token Action Bar", () => {
        ...
        ["toggleEquipped", "open-sheet", "prepareSpell", "rollSkill", "rollAttack", "rollSpell", "rollDefense"].forEach(
            (action) => {
        ...
        it("should contain items in the action bar", async () => {
            ...
            const allSpellDataSets = getDataSets(
                tokenActionBar.element.querySelectorAll("[data-action='prepareSpell']")
            );
            expect(allSpellDataSets.map((d) => d.spellId)).to.contain(spell.id);

            const allWeaponDataSets = getDataSets(
                tokenActionBar.element.querySelectorAll("[data-action='rollAttack']")
            );
        ...
        (
            ...
            it(`should display weapons for skill '${skill}' as ${preparedTitle}`, async () => {
                ...
                expect(allWeaponDataSets.find((d) => d.attackId == weapon.id)?.prepared).to.equal(
                    prepared ? "true" : "false"
                );
            });
        });
        ...
        ["defense", "bodyresist", "mindresist"].forEach((defenseType) => {
            it(`should trigger defense dialog for ${defenseType}`, async () => {
                ...
                const defenseElement = tokenActionBar.element.querySelector(
                    `[data-defense-type='${defenseType}'][data-action='rollDefense']`
                );
    ...
    describe("Tick Bar Hud", () => {
        ...
        function getCombatantItem(combatantId: string) {
            const combatantItem = document.querySelector(
                `.tick-bar-hud-combatant-list-item[data-combatant-id="${combatantId}"]`
            ) as HTMLElement;
        ...
        describe("_onRender", () => {
            it("should handle double-click to open character sheet", async () => {
                ...
                await passesEventually(
                    () => expect(token.actor.sheet.rendered, "Actor sheet was rendered").to.be.true,
                    500
                );
            });
            ...
            it("should handle single-click to control token", async () => {
                ...
                expect(controlSpy.calledWith({ releaseOthers: true }), "Token control called with correct parameters")
                    .to.be.true;
                expect(animatePanSpy.calledOnce, "Canvas animate pan was called").to.be.true;
                expect(
                    animatePanSpy.calledWith({
                        x: token.x,
                        y: token.y,
                    }),
                    "Canvas pan called with token coordinates"
                ).to.be.true;
            });
            ...
            it("should hide previous tick button when on current tick", async () => {
                ...
                const previousTickButton = tickBarHud.element.querySelector(
                    '.tick-bar-hud-nav-btn[data-action="previous-ticks"]'
                ) as HTMLElement;
