    ...
    describe("addHandler", () => {
        describe("successful handler registration", () => {
            it("should register a new handler for a given groupId", () => {
                ...
                expect(() => registry.addHandler("item", TestHandler)).to.throw(
                    "Modifier handler for groupId 'item' is already registered."
                );
            });
            ...
            it("should convert groupId to lowercase for registration", () => {
                ...
                expect(() => registry.addHandler("item", AnotherTestHandler)).to.throw(
                    "Modifier handler for groupId 'item' is already registered."
                );
            });
        ...
        describe("shadowing detection", () => {
            it("should debug log when registering more specific handler than existing one", () => {
                ...
                expect(debugMessage).to.include(
                    "Splittermond | More general handlers than groupId 'item.damage.fire':"
                );
            ...
            it("should debug log for multiple levels of nesting", () => {
                ...
                const debugMessage = consoleDebugSpy.lastCall.args.join(" ");
                expect(debugMessage).to.include(
                    "Splittermond | More general handlers than groupId 'item.damage.fire.magical.enhanced':"
                );
            });
            ...
            it("should debug log for partial matches at any level", () => {
                ...
                const debugMessage = consoleDebugSpy.lastCall.args.join(" ");
                expect(debugMessage).to.include(
                    "Splittermond | More general handlers than groupId 'actor.skills.combat':"
                );
        ...
        describe("case sensitivity variations", () => {
            it("should treat different cases as the same groupId", () => {
                ...
                expect(() => registry.addHandler("item.damage.fire", AnotherTestHandler)).to.throw(
                    "Modifier handler for groupId 'item.damage.fire' is already registered."
                );
