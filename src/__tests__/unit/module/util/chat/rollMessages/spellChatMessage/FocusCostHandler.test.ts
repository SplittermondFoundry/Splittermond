...
describe("FocusCostActionHandler", () => {
    ...
    describe("Degree of success option render", () => {
        ...
        it("should render all degree of success options that have isOption true (non-channeled)", () => {
            ...
            const expectedActions = ["consumedFocusUpdate", "exhaustedFocusUpdate", "spellEnhancementUpdate"];
        ...
        it("should render all degree of success options that have isOption true (channeled)", () => {
            ...
            const expectedActions = ["consumedFocusUpdate", "channeledFocusUpdate", "spellEnhancementUpdate"];
        ...
        (
            [
                ...
                ["exhaustedFocusUpdate", new Cost(1, 0, false)],
            ] as const
        ...
        (
            [
                ...
                ["exhaustedFocusUpdate", new Cost(1, 1, false)],
            ] as const
        ).forEach(([option, effect]) => {
        ...
        ["consumedFocusUpdate", "channeledFocusUpdate", "exhaustedFocusUpdate", "spellEnhancementUpdate"].forEach(
            (option) => {
                it(`should render the degree of success cost negatively for checked option ${option}`, () => {
                    ...
                    const renderedOption = options
                        .filter((o) => o.render.action === `${option}`)
        ...
        ["consumedFocusUpdate", "channeledFocusUpdate", "exhaustedFocusUpdate"].forEach((option) => {
            [1, 2, 4, 8].forEach((multiplicity) => {
                it(`should always offer checked option ${option} with multiplicity ${multiplicity}`, () => {
                    ...
                    underTest.spellReference
                        .getItem()
                        .getCostsForFinishedRoll.returns(new Cost(0, 0, false).asPrimaryCost());
    ...
    describe("useDegreeOfSuccessOption", () => {
        ...
        ["spellEnhancementUpdate", "consumedFocusUpdate", "channeledFocusUpdate", "exhaustedFocusUpdate"].forEach(
            (option) => {
    ...
    describe("FocusDegreeOfSuccessOptionField", () => {
        ...
        it("should toggle checked state correctly", () => {
            const field = FocusDegreeOfSuccessOptionField.initialize(
                true,
                1,
                parseCostString("1V1").asModifier(),
                "text"
            );
        ...
        it("should calculate cost and effect correctly for multiplicities", () => {
            const effect = new CostModifier({ _channeled: 0, _channeledConsumed: 0, _exhausted: -1, _consumed: -1 });
            const field = withToObjectReturnsSelf(() =>
                FocusDegreeOfSuccessOptionField.initialize(true, 1, effect, "text")
            );
...
function setUpFocusActionHandler(sandbox: SinonSandbox): WithMockedRefs<FocusCostHandler> {
    ...
    return withToObjectReturnsSelf(() => {
        const handler = FocusCostHandler.initialize(
            AgentReference.initialize(actorMock),
...
function setNecessaryDefaultsForSpellproperties(
