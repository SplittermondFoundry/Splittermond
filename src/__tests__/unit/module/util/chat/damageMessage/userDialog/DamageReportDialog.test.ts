...
describe("DamageReportDialog", () => {
    ...
    beforeEach(() => {
        ...
        let html = "";
        fakeRenderer.callsFake(() => async (template: string, data: unknown) => {
    ...
    it("should record increments", async () => {
        ...
        dom.querySelector("button.button-inline[data-action='inc-value']")?.dispatchEvent(
    ...
    it("should record a complex chain of increments", async () => {
        ...
        dom.querySelector("button.button-inline[data-action='dec-value']")?.dispatchEvent(
            newClickEvent(dialogStub.window)
        );
        dom.querySelector("button.button-inline[data-action='inc-value']")?.dispatchEvent(
            newClickEvent(dialogStub.window)
    ...
    it("should record half-value", async () => {
        const { dialogStub, dom } = await getDomUnderTest(sandbox, {
            totalDamage: new Cost(7, 0, false, true).asModifier(),
        });

        dom.querySelector("button.button-inline[data-action='half-value']")?.dispatchEvent(
            newClickEvent(dialogStub.window)
        );
    ...
    it("should record splinterpoint usage", async () => {
        ...
        dom.querySelector("button.dialog-buttons[data-action='useSplinterpoint']")?.dispatchEvent(
            newClickEvent(dialogStub.window)
        );
