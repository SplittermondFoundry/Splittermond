...
describe("damageApplicationHandlers", () => {
    ...
    describe("applyDamageToTargets", () => {
        it("should warn user if no target is selected", async () => {
            ...
            await damageHandlers.applyDamageToTargets(
                createDamageEvent(sandbox, { implements: [createDamageImplement(10, 1)] })
            );
        ...
        it("should apply damage to all targets", async () => {
            ...
            expect(dialogMock.getUserAdjustedDamage.args.map((arg) => arg[0].totalDamage.length)).to.deep.equal([
                report.totalDamage(),
                report.totalDamage(),
                report.totalDamage(),
            ]);
    ...
    describe("applyDamageToUserTargets", () => {
        ...
        it("should warn user if user is found", async () => {
            ...
            await damageHandlers.applyDamageToUserTargets(
                createDamageEvent(sandbox, { implements: [createDamageImplement(10, 1)] })
            );
    ...
    describe("applyDamageToSelf", () => {
        it("should warn user if no actor on user", async () => {
            ...
            await damageHandlers.applyDamageToSelf(
                createDamageEvent(sandbox, { implements: [createDamageImplement(10, 1)] })
            );
        ...
        it("should apply damage to self", async () => {
            ...
            expect(dialogMock.getUserAdjustedDamage.firstCall.args[0].totalDamage.length).to.deep.equal(
                report.totalDamage()
            );
            expect(
...
function createTargetActor(sandbox: SinonSandbox, id: number) {
    ...
    sandbox.stub(actor, "resistances").get(() => ({ display: "", calculate: () => Promise.resolve({ physical: 0 }) }));
    sandbox.stub(actor, "weaknesses").get(() => ({ display: "", calculate: () => Promise.resolve({ physical: 0 }) }));
    sandbox.stub(actor, "damageReduction").get(() => ({ display: "0", calculate: () => Promise.resolve(0) }));
    sandbox.stub(actor, "protectedDamageReduction").get(() => ({ display: "0", calculate: () => Promise.resolve(0) }));
    return actor;
}
