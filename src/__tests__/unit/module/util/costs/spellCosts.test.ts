...
import { calculateReducedEnhancementCosts, calculateReducedSpellCosts } from "module/util/costs/spellCosts.js";
...
describe("Spell cost calculation basics", () => {
    ...
    ["", null, undefined, [], 1, "0"].forEach((costs) => {
        ...
        it(`should handle a spell with invalid enhancement cost ${costs}`, () => {
            const spellData = {
                ...baseSpellData,
                enhancementCosts: costs as any /*we're simulationg invalid input here*/,
            };
            const reducedCosts = calculateReducedEnhancementCosts(spellData, noReductionSpellCostManager);
...
describe("Spell cost calculation with reductions", () => {
    ...
    (
        [
            ["cost reduction", calculateReducedEnhancementCosts],
            ["enhancement reduction", calculateReducedSpellCosts],
        ] as const
    ).forEach(([title, calculationFunction]) => {
...
describe("Spell cost calculation reduction selection", () => {
    ...
    it("should apply reductions only once per spell", () => {
        ...
        const reducedCosts = calculateReducedEnhancementCosts(
            spellData,
            reductionManagement.spellEnhancedCostReduction
        );
