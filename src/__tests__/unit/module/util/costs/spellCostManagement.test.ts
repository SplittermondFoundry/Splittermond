...
describe("Spell cost Management addition of reductions", () => {
    (
        ...
        it(`should be able to add a global cost modifier for ${title}`, () => {
            ...
            manager.addCostModifier({ label: "foreduction", value: mod("K2V1"), skill: null, attributes: {} });
        ...
        it(`should be able to add a skill specific cost modifier for ${title} for a skilled item`, () => {
            const manager = initializeSpellCostManagement({})[managerKey];
            manager.addCostModifier({ label: "foreduction", value: mod("K2V1"), skill: "deathmagic", attributes: {} });
...
describe("Spell cost Management multiple reductions", () => {
    (
        ...
        it(`should return all global reductions for ${title}`, () => {
            ...
            expect(manager.getCostModifiers("", "").reduce((a, b) => a.add(b), zero)).to.deep.equal(
                new Cost(3, 3, true).asModifier()
            );
        });
        ...
        it(`should return all skill specific reductions for ${title}`, () => {
            ...
            expect(manager.getCostModifiers("deathmagic", "").reduce((a, b) => a.add(b), zero)).to.deep.equal(
                new Cost(3, 3, true).asModifier()
            );
        });
        ...
        it(`should return all skill and type specific reductions for ${title}`, () => {
            ...
            expect(manager.getCostModifiers("deathmagic", "spirit").reduce((a, b) => a.add(b), zero)).to.deep.equal(
                new Cost(3, 3, true).asModifier()
            );
        });
    });
...
describe("Spell cost Management selection of reductions", () => {
    ...
    const management = initializeSpellCostManagement({});
    [management.spellCostReduction, management.spellEnhancedCostReduction].forEach((reductionManager) => {
    ...
    ([management.spellCostReduction, management.spellEnhancedCostReduction] as const).forEach((reductionManager) => {
        ...
        it("should return skill and type specific and global reductions if both are given", () => {
            ...
            expect(reductions).to.contain.all.members([
                deathmagicConjurationReduction,
                conjurationReduction,
                deathmagicReduction,
                globalReduction,
            ]);
        });
