...
describe("TickBarHud", () => {
    ...
    describe("_onRender", () => {
        ...
        it("should set z-index for combatant list children", async () => {
            ...
            const combatantList = underTest.element.querySelector(
                '.tick-bar-hud-tick[data-tick="10"] .tick-bar-hud-combatant-list'
            );
        ...
        it("should add hover animations to combatant lists", async () => {
            ...
            expect(animateStub.firstCall.firstArg).to.deep.equal([{ marginTop: "0px" }, { marginTop: "5px" }]);
            expect(animateStub.lastCall.firstArg).to.deep.equal([{ marginTop: "5px" }, { marginTop: "-38px" }]);
        });
        ...
        it("should add click handler to forward navigation button", async () => {
            ...
            const nextButton = underTest.element.querySelector(
                '.tick-bar-hud-nav-btn[data-action="next-ticks"]'
            ) as HTMLElement;
            ...
            const scrollAnimateCalls = animateStub
                .getCalls()
                .filter((call) => call?.thisValue?.classList.contains("tick-bar-hud-ticks-scroll"));
        ...
        it("should add click handler to backward navigation button", async () => {
            ...
            const prevButton = underTest.element.querySelector(
