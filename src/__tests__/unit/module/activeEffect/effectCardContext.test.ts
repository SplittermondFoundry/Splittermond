...
describe("buildEffectCardContext", () => {
    ...
    describe("isInactive", () => {
        it("all-active effect is not inactive", () => {
            const result = buildEffectCardContext(makeEffect());

            expect(result.isInactive).to.be.false;
        });

        it("disabled effect is inactive", () => {
            const result = buildEffectCardContext(makeEffect({ disabled: true }));

            expect(result.isInactive).to.be.true;
        });

        it("suppressed effect is inactive", () => {
            const result = buildEffectCardContext(makeEffect({ isSuppressed: true }));

            expect(result.isInactive).to.be.true;
        });

        it("expired effect is inactive", () => {
            const result = buildEffectCardContext(
                makeEffect({
                    durationMode: "timed",
                    duration: {
        ...
        it("autoGenerated effect is not inactive (S4: type no longer counts as inactive)", () => {
            const result = buildEffectCardContext(makeEffect({ type: "autoGenerated" }));

            expect(result.isInactive).to.be.false;
        });

        it("autoGenerated effect that is separately disabled is inactive", () => {
            const result = buildEffectCardContext(makeEffect({ type: "autoGenerated", disabled: true }));

            expect(result.isInactive).to.be.true;
        });

        it("disabled composes with other flags (orthogonal)", () => {
            const result = buildEffectCardContext(
                makeEffect({
                    disabled: true,
                    isSuppressed: true,
